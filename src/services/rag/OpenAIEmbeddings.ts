...
export class OpenAIEmbeddings implements Embeddings {
  ...
  async load(): Promise<this> {
    ...
    if (!key) {
      throw new Error('rag_openai_api_key_missing');
    }
    this.apiKey = key;
    this.loaded = true;
    return this;
  ...
  async embed(text: string): Promise<number[]> {
    ...
    if (!response.ok) {
      ...
      throw new Error(payload.includes('key') ? 'rag_openai_embed_denied' : 'rag_openai_embed_failed');
    }

    const json = await response.json();
    const vector = json?.data?.[0]?.embedding;
    if (!Array.isArray(vector) || vector.length === 0) {
      throw new Error('rag_openai_embed_empty');
    }
