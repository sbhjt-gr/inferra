...
function parseMessages(payload: any): Array<{ role: string; content: string }> {
...
export function createChatCompletionHandler(context: Context) {
  return async (method: string, path: string, body: string, socket: any): Promise<boolean> => {
    ...
    if (!payload || !Array.isArray(payload.messages)) {
      context.respond(socket, 400, { error: 'messages_required' });
      logger.logWebRequest(method, path, 400);
      return true;
    }
